...
from XRF_tomography_test_gpu_rev_rot import reconstruct_jXRFT_tomography
from array_ops_test_gpu_rev_rot import save_rotation_lookup
...
params_3d_64_64_64 = {'dev': dev,
                      ...
                      'minibatch_size': tc.tensor(512).to(dev),
                      'b': 1.0E-3,
                      'lr': 1.0E-3,
                      'init_const': 0.5,
                      'fl_line_groups': np.array(["K", "L", "M"]),
...
array_size = [params['sample_height_n'].item(), params['sample_size_n'].item(), params['sample_size_n'].item()]
theta_ls = - tc.linspace(params['theta_st'], params['theta_end'], params['n_theta']+1)[:-1]
dest_folder = os.path.join(params['recon_path'], "rotation_look_up")
