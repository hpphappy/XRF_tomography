...
def self_absorption_att_ratio_single_theta_3d(src_path, selfAb, n_det, P, det_size_cm, det_from_sample_cm, det_ds_spacing_cm, sample_size_n, sample_size_cm, sample_height_n, 
...
def create_XRF_data_single_theta_3d(selfAb, n_det, P, theta_st, theta_end, n_theta, src_path, det_size_cm, det_from_sample_cm, det_ds_spacing_cm, sample_size_n,
    ...
    SA_att_ratio = self_absorption_att_ratio_single_theta_3d(src_path, selfAb, n_det, P, det_size_cm, det_from_sample_cm, det_ds_spacing_cm, sample_size_n, sample_size_cm, sample_height_n, 
                                                                 this_aN_dic, probe_energy, dev, theta)
...
def create_XRF_data_3d(n_ranks, rank, selfAb, P_folder, f_P, theta_st, theta_end, n_theta, src_path, det_size_cm, det_from_sample_cm, det_ds_spacing_cm, sample_size_n,
    ...
    if rank == 0:
        if selfAb == True:
            ...
            P = 0
            n_det = 0  #This soesn't mean there's no detector. n_det is not reqired when not consider self-absorption
            
        theta_ls = - tc.linspace(theta_st, theta_end, n_theta + 1)[:-1] 
        for this_theta_idx, theta in enumerate(tqdm(theta_ls)):
            create_XRF_data_single_theta_3d(selfAb, n_det, P, theta_st, theta_end, n_theta, src_path, det_size_cm, det_from_sample_cm, det_ds_spacing_cm, sample_size_n,
                                 sample_size_cm, sample_height_n, this_aN_dic, probe_cts, probe_energy, save_path, save_fname, Poisson_noise, dev, this_theta_idx)
