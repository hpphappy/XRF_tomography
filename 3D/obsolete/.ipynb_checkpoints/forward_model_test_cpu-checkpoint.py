...
#         self.probe_attCS_dic = dict(zip(self.element_ls, self.probe_attCS_ls))
    ...
    def init_SA_theta(self):  ##can be written using python parallel?
        n_element = self.n_element.to('cpu')
        n_lines = self.n_lines.to('cpu')
        n_voxel_batch = self.n_voxel_batch.to('cpu')
        voxel_idx_offset = self.p * self.n_voxel_batch

        att_exponent = tc.stack([self.lac[:,:, tc.clamp((self.P_batch[m,0] - voxel_idx_offset.to('cpu')), 0, n_voxel_batch).to(dtype=tc.long), self.P_batch[m,1].to(dtype=tc.long)]
                                 * self.P_batch[m,2].view(1, 1, -1).repeat(n_element, n_lines, 1) for m in range(self.n_det)])
        ...
        att_exponent_voxel_sum = tc.sum(att_exponent.view(self.n_det, n_element, n_lines, n_voxel_batch, self.dia_len_n), axis=-1)
        ...
        SA_theta =  tc.mean(tc.exp(-tc.sum(att_exponent_voxel_sum, axis=1)), axis=0).to(self.dev)    
