...
from array_ops_mpi import initialize_guess_3d
from forward_model_mpi import PPM, PPM_cont
...
def reconstruct_jXRFT_tomography(dev, recon_idx, cont_from_check_point, use_saved_initial_guess, recon_path, f_initial_guess, f_recon_grid,
    ...
    minibatch_ls_0 = tc.arange(n_ranks)
...
#         P = P.view(n_det, 3, dia_len_n * sample_height_n * sample_size_n * sample_size_n) 
    ...
    if cont_from_check_point == False: 
        ...
        #         print("this_theta_idx = %d" %(this_theta_idx))
                ...
                for m in range(n_batch):
                    ...
                    if rank == 0:
                        ...
                        P_batch = tc.clone(P[:,:, p * dia_len_n * minibatch_size * sample_size_n :  (p + n_ranks) * dia_len_n * minibatch_size * sample_size_n])
                        P_batch = P_batch.view(n_det, 3, n_ranks, dia_len_n * minibatch_size * sample_size_n)
                        P_batch = P_batch.permute(2,0,1,3)

                    else:
                        P_batch = None
           
                    comm.Barrier()
                    P_batch = comm.scatter(P_batch, root=0)
                
                    if epoch == 0 and this_theta_idx == 0 and m == 0:
                    ...
                    # %memit
                    # print("current reserved memory: %g GB" %(tc.cuda.memory_stats(dev)['reserved_bytes.all.current']/1.0E9))
                    updated_minibatch = model.xp.detach()
                    updated_batch = comm.gather(updated_minibatch,root=0)
                    
                    ## Note that the gathered updated_batch is a list of tensor, namely a list with n_ranks tansors. (n_ranks, n_element, minibactch_size, sample_size_n)
                    ...
                    if rank == 0: 
                        updated_batch = tc.cat(updated_batch).view(n_ranks, n_element, minibatch_size, sample_size_n)                        
                        updated_batch = updated_batch.permute(1,0,2,3)
                        updated_batch = tc.reshape(updated_batch, (n_element, n_ranks * minibatch_size, sample_size_n))
                        concentration_map_rot[:, minibatch_size * p : minibatch_size * (p + n_ranks),:] = updated_batch.detach()
                        
                        X_update = rotate(concentration_map_rot.view(n_element, sample_height_n, sample_size_n, sample_size_n), -theta, dev).detach()
                        del concentration_map_rot
        ...
        if rank == 0:    
            ...
            fig6_ax1.plot(loss_epoch.detach().numpy())
            ...
            np.save(os.path.join(recon_path, 'loss_epoch.npy'), loss_epoch.detach().numpy())
            np.save(os.path.join(recon_path, 'mse_model_elements.npy'), mse_epoch.detach().numpy())
...
#                     tc.cuda.empty_cache()
        ...
        if rank == 0:    
            ...
            for i in range(X.shape[0]):
                fig7_ax1 = fig7.add_subplot(gs7[0,i])
                fig7_ax1.plot(mse_epoch_cont[:,i].detach().numpy())
                fig7_ax1.set_xlabel('epoch')
                fig7_ax1.set_ylabel('mse of model (each element)')
                fig7_ax1.set_title(str(list(this_aN_dic.keys())[i]))
                fig7_ax1.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.2f')) 
