...
def reconstruct_jXRFT_tomography(dev, recon_idx, cont_from_check_point, use_saved_initial_guess, recon_path, f_initial_guess, f_recon_grid,
    ...
    n_batch = tc.ceil((sample_height_n * sample_size_n).float()/(n_minibatch * minibatch_size).float()).int()
    full_batch = (sample_height_n * sample_size_n) % (n_minibatch * minibatch_size) == 0
