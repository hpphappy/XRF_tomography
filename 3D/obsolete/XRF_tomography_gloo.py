...
set_target_gpu(1)
@profile
def reconstruct_jXRFT_tomography(rank, world_size, dev, recon_idx, cont_from_check_point, use_saved_initial_guess, recon_path, f_initial_guess, f_recon_grid,
    ...
    if cont_from_check_point == True:
        ...
        mse_epoch_tot_cont = tc.zeros(n_epoch, len(this_aN_dic)).to(dev)
        ...
        for epoch in tqdm(range(n_epoch)):
            for this_theta_idx, theta in enumerate(theta_ls):
                ...
                for m in range(n_batch):
                    ...
                    print(minibatch_ls)
                    loss_fn = nn.MSELoss()
                    model = [PPM_cont(X, p, len(this_aN_dic), sample_height_n, minibatch_size, sample_size_n, sample_size_cm,
                    this_aN_dic, probe_energy, probe_cts, fl_line_groups, fl_K, fl_L, fl_M, group_lines,
                    theta_st, theta_end, n_theta, this_theta_idx,
                    det_ds_spacing_cm, det_size_cm, det_from_sample_cm, P_save_path).to(dev) for p in minibatch_ls]
                    tc.cuda.empty_cache()
    ...
    #                     loss = XRF_loss
            ...
            mse_epoch_tot_cont[epoch] = tc.mean(tc.square(X - X_true).view(X.shape[0], X.shape[1]*X.shape[2]*X.shape[3]), dim=1)
            tqdm._instances.clear() 
