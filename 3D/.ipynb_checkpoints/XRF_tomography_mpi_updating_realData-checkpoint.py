...
#                     print_flush_root(rank, val=minibatch_ls, output_file='minibatch_ls.csv', **stdout_options)
                ...
                if rank == 0:
                    loss_whole_obj[n_theta * epoch + idx] = tc.mean(loss_n_batch)
                    X_cpu = X.cpu()
                else:
                    X_cpu = None
        ...
        if rank == 0:            
            ...
            gs6 = gridspec.GridSpec(nrows=1, ncols=1, width_ratios=[1])
