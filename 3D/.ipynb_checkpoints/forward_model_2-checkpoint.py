...
class PPM(nn.Module):
    ...
    def init_probe(self):       
        probe_before_attenuation = (self.probe_cts.view(self.probe_cts.shape[0],1,1)) * (tc.ones(self.minibatch_size, self.sample_size_n, device = self.dev).repeat(self.probe_cts.shape[0],1,1))
        return probe_before_attenuation.view(self.probe_cts.shape[0], self.n_voxel_minibatch)
...
#         transmission_theta = tc.exp(-att_exponent_acc_map[:,-1])
        ...
        fl_signal_SA_theta = probe_after_attenuation_theta * fl_map_tot_flat_theta * self.SA_theta  
