...
def intersecting_length_fl_detectorlet_3d(det_size_cm, det_from_sample_cm, det_ds_spacing_cm, sample_size_n, sample_size_cm, sample_height_n, P_save_path):
    ...
    if os.path.isfile(P_save_path):
        print("\n The file containing the intersecting lengths is found. Loading ...")
        ...
        print("\n The file containing the intersecting lengths is not found. Calculating ...")
        ...
        ### convert the unit of spacing between detector points to the number of the sample voxels
        ...
        for i,  det_pos in enumerate(det_pos_ls_flat):
            print("Calculating the intersecting information for the detector point: %d / %d" %(i, len(det_pos_ls_flat)))
            for j, v in enumerate(tqdm(voxel_pos_ls_flat)): 

                # Solving the intersection of the ray with the sample boundary along axis-1 (x_axis)
                bdx_int = trace_beam_x(v[0], v[1], v[2], det_pos[0], det_pos[1], det_pos[2], sample_x_edge)

                # Solving the intersection of the ray with the sample boundaries along axis-2 (y_axis) and axis-0 (z_axis),
                # we will get 2 solutions for each axis since there're 2 bdry planes on each axis
                ...
                int_idx_flat = int_idx[:,0] * (sample_size_n.item() * sample_size_n.item()) + int_idx[:,1] * sample_size_n.item() + int_idx[:,2]
